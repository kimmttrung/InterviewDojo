...
export const CodeforcesLanguages = {
  typeId,
  extensions,
  getExtension(language: string = 'text'): string {
    const lang = language.toLowerCase();
    if (lang.includes('c++') || lang.includes('g++')) return 'cpp';
    if (lang.includes('gcc')) return 'c';
    if (lang.includes('java')) return 'java';
    if (lang.includes('python')) return 'py';
    if (lang.includes('javascript') || lang.includes('v8')) return 'js';
    return 'txt';
  },
};
