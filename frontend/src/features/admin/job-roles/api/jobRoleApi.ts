...
import { JobRole } from '../../questions/types/question.types';
...
export const jobRoleApi = {
  ...
  update: (id: number, data: { name?: string; description?: string }): Promise<JobRole> =>
    api.put(`/job-roles/${id}`, data).then((res) => res.data.data),

  delete: (id: number): Promise<void> => api.delete(`/job-roles/${id}`),
};
